...
import { duplicateAPI, signUpAPI } from "../../apis/authAPI";
...
const SignUp = () => {
  ...
  const duplicateID = async () => {
    const result = await duplicateAPI(loginId);
    console.log(result);
  };
  ...
  const signUp = async () => {
    ...
    const result = await signUpAPI({ loginId, name, password });
    console.log(result);
  };
      ...
      <div className="flex flex-wrap items-center justify-center h-screen">
        <div className="lg:w-2/5 md:w-2/3 bg-gray-100 rounded-lg p-8 flex flex-col w-full ">
          ...
          <div className="flex flex-row mb-4 justify-between">
            ...
            <button
              onClick={duplicateID}
              className="w-1/5 bg-blue-500 text-white py-1 px-3 leading-8 rounded text-sm font-semibold hover:bg-blue-700"
            >
              중복확인
            </button>
          </div>
